...
class TransitionCollectionWithCallbacksTest < StateMachinesTest
  def setup
    ...
    @state.around_transition do |block|
      @before_callbacks << :state_around
      block.call
      @after_callbacks << :state_around
    end
    ...
    @status.around_transition do |block|
      @before_callbacks << :status_around
      block.call
      @after_callbacks << :status_around
    end
